...
from application.api.posts import (
    get_all_unpublished_posts,
...
def schedule_bot_jobs(scheduler: AsyncIOScheduler, bot: Bot):
    ...
    posts = get_all_unpublished_posts()
    ...
    for post in posts:
        ...
        if post.scheduled_time > now:
            ...
            logging.info(f"Scheduled job for post_id: {post.post_id} at {post.scheduled_time}")
        else:
            # Если время уже прошло, логируем и пропускаем пост
            logging.warning(f"Skipping post_id: {post.post_id} because its scheduled time {post.scheduled_time} is in the past.")
